	...
	<div id="wrapper" class="container-fluid">
		<div class="row">
			...
			<div class="col-md-2 col-sm-2 col-xs-4 sidebar-wrapper"
				id="sidebar-wrapper">
				<span class="sidebar-brand"><s:property value="user.getId()" />
				</span>
				<form id="input-form">
				...
				<ul class="sidebar-nav" id="notebooks">
					<s:iterator value="books" var="book">
						<li  name=<s:property value="#book.getId()"/>><a
							href="#" class="menu-toggle"><s:property value="#book.getTitle()" /> </a>
						</li>
					</s:iterator>
				</ul>
			</div>
...
-->
		...
		<script>
			...
			function editNotebook(target) {
				$.post("updateNotebook", {
					"notebook.id" : target.getAttribute("name"),
					"notebook.title" : target.val()
				}, function(data) {
					$.each(data, function(i, note) {
						$("#title").val(note.title);
						$("#txtEditor").Editor("setText", note.content);
